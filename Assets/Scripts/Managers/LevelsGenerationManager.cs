...
public class LevelsGenerationManager : Singleton<LevelsGenerationManager>
{
    ...
    public List<Container> _containersToPlace2 = new List<Container>();
    ...
    void SortContainers()
    {
        ...
        foreach (var order in currentLevelGenerated.orders)
        {
            List<Container_Level> levelContainers = new List<Container_Level>();

            //Basique -> Cooled -> Tank - Fire
            foreach (var levelContainer in order.levelContainers)
                if (levelContainer.containerType == ContainerType.Basique)
                    levelContainers.Add(levelContainer);

            foreach (var levelContainer in order.levelContainers)
                if (levelContainer.containerType == ContainerType.Réfrigéré)
                    levelContainers.Add(levelContainer);

            foreach (var levelContainer in order.levelContainers)
                if (levelContainer.containerType == ContainerType.Citerne)
                    levelContainers.Add(levelContainer);

            foreach (var levelContainer in order.levelContainers)
                if (levelContainer.containerType == ContainerType.Dangereux)
                    levelContainers.Add(levelContainer);


            foreach (var levelContainer in levelContainers)
            {
                int chosenContainerIndex = 0;

                for (int i = siblingIndex; i < _containersToPlace.Count; i++)
                {
                    if (_containersToPlace[i].containerColor != levelContainer.containerColor)
                        continue;

                    if (_containersToPlace[i].containerType != levelContainer.containerType)
                        continue;

                    if (_containersToPlace[i].isDoubleSize != levelContainer.isDoubleSize)
                        continue;

                    chosenContainerIndex = i;

                    break;
                }
               
                var chosenContainer = _containersToPlace[chosenContainerIndex];
                _containersToPlace.RemoveAt(chosenContainerIndex);

                _containersToPlace.Insert(siblingIndex, chosenContainer);

                siblingIndex++;
            }
        }

        _containersToPlace2 = new List<Container>(_containersToPlace);
    }
    ...
    void FillContainerZone(Transform containersParent, Transform spotsParent, int pileCount, int zoneMaxFilling, int percentageToFill = 100, bool forceSpawnDoubleFirst = false)
    {
        ...
        while (containersPercentage < percentageToFill && zoneFillingPercentage <= zoneMaxFilling && containersToPlace.Count > 0)
        {
            ...
            Container container = containersToPlace[containersToPlace.Count - 1];
